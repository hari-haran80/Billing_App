...
const addMissingColumns = async () => {
  ...
  try {
    ...
    const hasLastPricePerUnit = tableInfo.some(col => col.name === 'last_price_per_unit');
    
    if (!hasLastPricePerUnit) {
      console.log('[DB] Adding last_price_per_unit column to items table');
      await db.runAsync('ALTER TABLE items ADD COLUMN last_price_per_unit REAL DEFAULT 0');
    }
    
    // Check if price_per_unit column exists in bill_items table
    const billItemsInfo = await db.getAllAsync('PRAGMA table_info(bill_items)');
    const hasPricePerUnit = billItemsInfo.some(col => col.name === 'price_per_unit');
    
    if (!hasPricePerUnit) {
      console.log('[DB] Adding price_per_unit column to bill_items table');
      await db.runAsync('ALTER TABLE bill_items ADD COLUMN price_per_unit REAL DEFAULT 0');
    }
    ...
    if (!hasQuantity) {
      ...
      await db.runAsync('ALTER TABLE bill_items ADD COLUMN quantity INTEGER DEFAULT 1');
    }
    ...
    if (!hasLWeight) {
      ...
      await db.runAsync('ALTER TABLE bill_items ADD COLUMN l_weight REAL DEFAULT 0');
    }
    
    // Check if weight_mode column exists in bill_items table
    const hasWeightMode = billItemsInfo.some(col => col.name === 'weight_mode');
    
    if (!hasWeightMode) {
      console.log('[DB] Adding weight_mode column to bill_items table');
      await db.runAsync('ALTER TABLE bill_items ADD COLUMN weight_mode TEXT DEFAULT \'normal\'');
    }
    
    // Check if reduced_weight column exists in bill_items table
    const hasReducedWeight = billItemsInfo.some(col => col.name === 'reduced_weight');
    
    if (!hasReducedWeight) {
      console.log('[DB] Adding reduced_weight column to bill_items table');
      await db.runAsync('ALTER TABLE bill_items ADD COLUMN reduced_weight REAL DEFAULT 0');
    }
  } catch (error) {
...
export const deleteItem = async (id: number) => {
  ...
  const billItems = await db.getAllAsync(
...
export const deleteBottleType = async (id: number) => {
  ...
  const bottle = await db.getFirstAsync(
...
export const deleteMultipleItems = async (ids: number[]) => {
  ...
  for (const id of ids) {
    ...
    if (billItems[0]?.count > 0) {
      throw new Error(`Cannot delete item with ID ${id}. It is used in existing bills.`);
    }
  }
...
export const saveBill = async (billData: any) => {
  ...
  const { billNumber, customerName, customerPhone, totalAmount, items } = billData;
  ...
  const result = await db.runAsync(
    ...
    [billNumber, customerName, customerPhone, totalAmount]
  );
    ...
    if (itemDetails?.unit_type === 'count') {
      ...
      await db.runAsync(
        `INSERT INTO bill_items 
         (bill_id, item_id, original_weight, quantity, final_weight, price_per_unit, amount, reduced_weight)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [billId, item.itemId, 0, item.quantity || 1, 0, item.price, item.amount, 0]
      );
...
export const getBillDetails = async (billId: number) => {
  ...
  const bill = await db.getFirstAsync('SELECT * FROM bills WHERE id = ?', [billId]);
