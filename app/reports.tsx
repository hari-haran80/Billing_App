...
export default function ReportsScreen() {
  ...
  const filterBills = () => {
    ...
    if (filters.customer) {
      filtered = filtered.filter(bill =>
        bill.customer_name.toLowerCase().includes(filters.customer.toLowerCase())
      );
    }
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      Alert.alert('Export Failed', error.message || 'Failed to export report. Please try again.');
    } finally {
      setExporting(false);
    }
  };
  ...
  const exportSummary = async () => {
    ...
    try {
      ...
      const summaryData = [
        ...
        [''],
        ['Report Period', `${minDate.toLocaleDateString()} to ${maxDate.toLocaleDateString()}`],
      ...
      Alert.alert('Export Failed', error.message || 'Failed to export summary report. Please try again.');
    } finally {
      setExporting(false);
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.resultsHeader}>
        ...
        <Text style={styles.resultsTotal}>
          Total: ₹{filteredBills.reduce((sum, bill) => sum + bill.total_amount, 0).toFixed(2)}
        </Text>
      </View>
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateSubtext}>
              {bills.length === 0 ? 'Create your first bill' : 'Try different filters'}
            </Text>
          </View>
        }
      />
